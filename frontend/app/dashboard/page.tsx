...
const typeConfig: Record<string, { color: string; bg: string; icon: typeof Info }> = {
  ...
  info:    { color: "text-emerald-400",    bg: "bg-emerald-400/10",    icon: Info          },
...
function InsightsCard({ incidents, cpuData, pendingCount }: {
  ...
  if (recommendations.length === 0) recommendations.push({ label: "All clear — run a pipeline scan to stay ahead", href: "/dashboard" });
          ...
          <div className="space-y-1.5 sm:border-l sm:border-slate-800 sm:pl-4">
            ...
            {recommendations.map((rec) => (
              <Link key={rec.label} href={rec.href}
                className="block text-xs text-emerald-400 hover:text-emerald-300 transition-colors">
                → {rec.label}
              </Link>
            ))}
          </div>
...
function SlackActivityFeed({ incidents }: { incidents: IncidentRecord[] }) {
  const activities = incidents.slice(0, 5).flatMap((inc) => {
    ...
    const rows = [
      ...
      { time: fmt(new Date(base.getTime() + 120000)),      label: "Fix Plan Ready",      color: "bg-emerald-500"    },
    ];
...
export default function Dashboard() {
  ...
  const kpiConfig = [
    ...
    { title: "AI Agents",          value: agents.length,   icon: Bot,           color: "text-emerald-400",   borderColor: "border-emerald-400/20",   trend: running ? `${runningAgents}/${agents.length} running now` : `idle · last run ${agentSeen}`, href: null },
    ...
    <div className="text-slate-100">
      <main className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <button onClick={() => runAgent()} disabled={running}
              className={`rounded-lg px-5 py-2 text-sm font-semibold text-white transition-colors ${running ? "cursor-not-allowed bg-slate-700" : "bg-emerald-600 hover:bg-emerald-500 shadow-lg shadow-emerald-600/20"}`}>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {kpiConfig.map((kpi) => {
            const card = (
              <Card className={`${kpi.borderColor} transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:shadow-emerald-900/20 ${kpi.href ? "cursor-pointer" : ""}`}>
                ...
                <AreaChart data={cpuData}>
                  <defs>
                    <linearGradient id="cpuGrad" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="5%"  stopColor="#34e89a" stopOpacity={0.3} />
                      <stop offset="95%" stopColor="#34e89a" stopOpacity={0}   />
                    </linearGradient>
                  </defs>
                  ...
                  <Area type="monotone" dataKey="usage" stroke="#34e89a" strokeWidth={2} fill="url(#cpuGrad)" name="CPU Usage" />
                </AreaChart>
              </ResponsiveContainer>
              )}
            </CardContent>
            ...
            <CardContent>
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={netData.length > 0 ? netData : networkData}>
                  ...
                  <Area type="monotone" dataKey="inbound"  stroke="#6ee7b7" strokeWidth={2} fill="url(#inGrad)"  name="Inbound"  />
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <Link href="/incidents" className="text-xs text-emerald-400 hover:text-emerald-300 transition-colors">View all →</Link>
            </CardHeader>
            <CardContent>
        ...
        {events.some((e) => e.event === "pipeline_complete") && (
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            {[
              ...
              { label: "Root Cause",      value: "Generated ✓",     color: "text-emerald-400"    },
