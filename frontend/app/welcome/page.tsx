...
import {
  ...
  ShieldCheck, DollarSign, Sparkles, Code2, ChevronDown,
} from "lucide-react";
...
export default function WelcomePage() {
  ...
  useEffect(() => {
    try {
      if (localStorage.getItem("cloudpilot_user")) setDashboardHref("/");
    } catch {}
  }, []);
  ...
  return (
    <div className="relative bg-black text-slate-100">
      ...
      {/* Top bar */}
      <header className="fixed top-0 inset-x-0 z-20 flex items-center justify-between px-6 py-5 pointer-events-none">
        ...
        <a
          href={dashboardHref}
          className="pointer-events-auto rounded-full border border-emerald-400/30 bg-emerald-400/10 px-4 py-1.5 text-xs font-semibold text-emerald-300 backdrop-blur-sm transition-colors hover:bg-emerald-400/20"
        >
          Enter dashboard
        </a>
      </header>
        ...
        <section className="flex h-screen flex-col items-center justify-center px-6 text-center">
          <Reveal>
            ...
            <div className="mt-9 flex flex-wrap items-center justify-center gap-3">
              ...
              <a
                href="https://github.com/tanishagargcoder/cloudpilot-ai"
                target="_blank"
                rel="noreferrer"
                className="pointer-events-auto inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-6 py-3 text-sm font-medium text-slate-200 backdrop-blur-sm transition-colors hover:bg-white/10"
              >
          ...
          <div className="absolute bottom-10 flex flex-col items-center gap-1.5 text-[10px] uppercase tracking-[0.3em] text-emerald-300/40">
            scroll
