...
void main(){ vec2 p = gl_PointCoord - 0.5; float l = length(p); if (l > 0.5) discard;
  float tex = smoothstep(0.5, 0.0, l); gl_FragColor = vec4(uColor * tex, tex * vA * 0.6); }`,
    ...
    /* ── Resize ───────────────────────────────────────────────────────────── */
    const resize = () => {
      // Fall back to the canvas box: the window reports 0 while the tab is
      // still hidden, which would otherwise leave a 0x0 drawing buffer.
      ...
      atmo.mat.uniforms.uRes.value.set(w * dpr, h * dpr);
      readScroll();
    };
